    ...
    class WP_GSF_Admin_Notifications {
        ...
        public function display_notifications() {
            ...
            $notifications = array_filter( $notifications, function($value, $key) use ($dismissed) {
                    $notice_id = $value['notification_id'];
                    if ( ! in_array( $notice_id, array_keys( $dismissed ) ) ) {
                        return true;
                    }
                    $dismiss_until = $dismissed[ $notice_id ];
                    if ( $dismiss_until === 'forever' ) {
                        return false;
                    }
                    return ! empty( $dismiss_until ) && time() > strtotime( $dismiss_until );
                    ...
                    echo "<div class='gsf-wp-carousel-container'>";
                        foreach ($notifications as $notification) {
                            $this->render_notification_item(
                                ...
                                isset($notification['forever_dismissible']) ? (bool) $notification['forever_dismissible'] : false
                            );
                        }
                        echo "</div>";
                        echo "<div class='gsf-wp-carousel-controls'>";
                            echo "<div class='gsf-wp-notification-dismiss'>
                                ...
                                <ul class='gsf-wp-notification-dropdown-menu'>
                                    ...
                                    <li class='gsf-wp-notification-forever-dismiss' style='display:none;'><a href='javascript:void(0);' class='gsf-wp-notification-dismiss-btn' data-days='forever'>Forever</a></li>
                                </ul>
                            </div>";
        ...
        private function render_notification_item($title, $notification_id, $message, $type, $dismissible = true, $icon = '', $forever_dismissible = false, $is_active = true) {
            ...
            $forever_attr = $forever_dismissible ? '1' : '0';

            echo "<div class='gsf-wp-carousel-slide {$classes}' data-type='{$type}' data-id='{$notification_id}' data-forever-dismissible='{$forever_attr}'>
        ...
        public function gsf_dismiss_notice() {
            ...
            if ( $dismiss_days === 'forever' ) {
                $dismissed[ $gsf_notice_id ] = 'forever';
            } else {
